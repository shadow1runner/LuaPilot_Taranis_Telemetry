...
local BattLevelmAh = 6200 --if 0 BatteryLevel calc from Volt else from this mAh Value        #
...
-- ###############################################################
     if HeadingOrDist == 1 or (DisplayTimer==1 and HeadingOrDist == 2)  then
       ...
       if settings['imperial'] ~=0 then
         drawText(167,0, "Dist: "..(round(gps_hori_Distance*3.28)).."f",SMLSIZE)
       else
        drawText(167,0, "Dist: "..(round(gps_hori_Distance)).."m",SMLSIZE)
      end
...
-- ############################################################### 
  ...
  if data.spd > 10 then --draw wh per km
    ...
    if settings['imperial']==0 then
     ...
     drawText(114, 58, " draw: "..(round(effizient,1))..'W', SMLSIZE)
   end
